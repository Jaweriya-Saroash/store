...
function normalizeOrder(input){
  ...
  const items=Array.isArray(input?.items)?input.items.map(x=>({
    ...
    price:clean(x.price,50),
    usdPrice:clean(x.usdPrice,50),
    oldPrice:clean(x.oldPrice,50),
    oldUsdPrice:clean(x.oldUsdPrice,50),
    qty:Math.max(1,Math.min(999,Number(x.qty)||1))
  })).filter(x=>x.name):[];

  const total=Number(input?.total||0);
  const usdTotal=Number(input?.usdTotal||0);

  const deliveryCharge={
    pkr:Number(input?.deliveryCharge?.pkr||0)||0,
    usd:Number(input?.deliveryCharge?.usd||0)||0
  };

  if(
    !clean(customer.name,160) ||
    !clean(customer.contact,100) ||
    !clean(customer.address,2000) ||
    !items.length
  ){
    throw new Error('Please complete the required order fields.');
  }
  ...
  return {
    ...
    customer:{
      ...
      contact:clean(customer.contact,100),
      email:clean(customer.email,254),
      address:clean(customer.address,2000),
      country:clean(customer.country,100),
      countryName:clean(customer.countryName,150),
      city:clean(customer.city,150)
    },
    items,
    total:Number.isFinite(total)?total:0,
    currency:clean(input?.currency,20)||'PKR'
  };
}
...
function nextOrderNumber(orders){
  ...
  for(const o of orders){
    const n=Number.parseInt(String(o?.orderNumber||''),10);
...
async function putOrdersAtomically(orders,message,expectedSha){
  ...
  const content=Buffer
    .from(JSON.stringify(orders,null,2)+'\n','utf8')
    .toString('base64');
  ...
  return gh(
    `/repos/${cfg.owner}/${cfg.repo}/contents/${apiPath(path)}`,
    {
      method:'PUT',
      headers:{
...
async function saveOrders(
  ...
  for(let attempt=0;attempt<4;attempt++){
    try{
      return await putOrdersAtomically(
        candidate,
        message,
        currentSha
      );
      ...
      if(idx<0){
        const e=new Error('Order no longer exists.');
        e.status=404;
        throw e;
      }
      ...
      latest[idx].statusHistory=
        Array.isArray(latest[idx].statusHistory)
          ?latest[idx].statusHistory
          :[];

      latest[idx].statusHistory.push({
        status,
        at:new Date().toISOString()
      });
...
async function sendEmail(order){
  ...
  if(!key){
    throw new Error(
      'Email service is not configured. Add RESEND_API_KEY in Vercel.'
    );
  }
  ...
  const items=order.items.map(
    (x,i)=>
      `${i+1}. ${x.name} × ${x.qty} — ${order.currency} ${Number(x.price||0).toLocaleString()}`
  ).join('\n');
  ...
  const text=[
    ...
    c.city
      ? `City: ${c.city}`
      : '',
    `Delivery charge: ${order.currency} ${Number(order.deliveryCharge?.pkr||0).toLocaleString()} · USD ${Number(order.deliveryCharge?.usd||0).toLocaleString()}`,
    '',
    'Order details:',
    items,
    '',
    `Total: ${order.currency} ${Number(order.total||0).toLocaleString()} · USD ${Number(order.usdTotal||0).toLocaleString()}`
  ].join('\n');
  ...
  const response=await fetch(
    'https://api.resend.com/emails',
    {
      ...
      headers:{
        ...
        'Content-Type':'application/json',
        'Idempotency-Key':`js-collections-order-${order.id}`
      },
      body:JSON.stringify({
        from,
        to:['jsofficialcollections@gmail.com'],
  ...
  if(!response.ok){
    ...
    throw new Error(
      `Email delivery failed (${response.status}). ${body.slice(0,250)}`
    );
  }
}
...
module.exports=async function handler(req,res){
  ...
  if(req.method==='GET'){
    try{
      ...
      if(orderNumber && pin){
        const current=await getJsonFile(dataPath('orders'));
        const orders=Array.isArray(current.data)
          ?current.data
          :[];

        const order=orders.find(
          o=>
            String(o.orderNumber)===orderNumber &&
            String(o.pin)===pin
        );

        if(!order){
          return res.status(404).json({
            error:'Order not found. Check your order number and PIN.'
          });
        }
        ...
        return res.status(200).json({
          order:{
            ...
            customer:{
              ...
              countryName:order.customer.countryName,
      ...
      const result=await getJsonFile(
        dataPath('orders')
      );
  ...
  if(req.method==='PATCH'){
    try{
      ...
      const current=await getJsonFile(
        dataPath('orders')
      );
      ...
      orders[idx].statusHistory=
        Array.isArray(orders[idx].statusHistory)
          ?orders[idx].statusHistory
          :[];
  ...
  try{
    ...
    try{
      ...
      emailWarning=
        emailErr.message ||
        'Order was saved, but the notification email could not be sent.';
    }
    ...
    return res.status(err.status||500).json({
      error:err.message||'Unable to submit order.'
    });
  }
};
